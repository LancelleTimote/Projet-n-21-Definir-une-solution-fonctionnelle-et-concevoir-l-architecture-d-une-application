...
export class AppComponent implements OnInit, OnDestroy {
  ...
  customerObject: Customer | undefined = {
    customerid: 0,
    address: '',
    userid: 0,
  };
  customerServiceObject: CustomerService | undefined = { customerserviceid: 0 };
  ...
  constructor(
    ...
    this.customerServiceObject = JSON.parse(
      sessionStorage.getItem('customerServiceObject')!
    );
    this.loggedUser = JSON.parse(sessionStorage.getItem('loggedUser')!);
    if (
      this.userObject &&
      this.loggedUser &&
      (this.customerObject || this.customerServiceObject)
    ) {
    ...
    this.stompClient?.unsubscribe(
      `/topic/message_sent/${this.currentConversationId}`
    );
  }
  ...
  selectUser(id: number): void {
    ...
    (this.customerServiceObject = undefined), (this.loggedUser = undefined);
    ...
    this.selectUserSubscription = this.usersService
      .get(id)
      .subscribe((response: UserResponse) => {
        ...
        if (userType === 'customer') {
          ...
          this.displayLoggedUser = `${firstname} ${lastname}, id: ${id}, usertype:${usertype}, customer_id:${customerid}`;
          this.findUserConversations(id);
          sessionStorage.setItem('userObject', JSON.stringify(this.userObject));
          sessionStorage.setItem(
            'customerObject',
            JSON.stringify(this.customerObject)
          );
          ...
          this.displayLoggedUser = `${firstname} ${lastname}, id: ${id}, usertype:${usertype}, customer_service_id:${customerserviceid}`;
          this.findUserConversations(id);
          sessionStorage.setItem('userObject', JSON.stringify(this.userObject));
          sessionStorage.setItem(
            'customerServiceObject',
            JSON.stringify(this.customerServiceObject)
          );
          sessionStorage.setItem('loggedUser', JSON.stringify(this.loggedUser));
          this.connect();
        }
  ...
  connect(): void {
    this.socket = new SockJS('http://localhost:3001/socket');
  ...
  connectionSuccess(): void {
    this.currentConversation = JSON.parse(
    ...
    this.newConversationSubscription = this.stompClient.subscribe(
      `/user/${
        ...
        this.customerServiceObject?.customerserviceid
      }/new_private_conversation/${this.loggedUser?.usertype}`,
      (callback: any) => {
        this.onNewPrivateConversation(callback.body);
      },
      {
        id: `/new_conversation/${this.loggedUser?.usertype}/${
          this.customerObject?.customerid ||
          this.customerServiceObject?.customerserviceid
        }`,
      }
  ...
  onNewPrivateConversation(payload: any): void {
    ...
    if (this.loggedUser?.usertype === 'customer') {
      ...
      this.conversations = [conversation, ...this.conversations!].sort(
        (a, b) => {
          return (
    ...
    } else if (this.loggedUser?.usertype === 'customer_service') {
  ...
  onConversationNewMessageSent(payload: any): void {
    ...
    let conv = this.conversations?.filter(
  ...
  sendMessage(): void {
    if (this.stompClient) {
      ...
      this.stompClient.send(
        `/app/sendMessage/${request.conversationid}`,
        {},
  ...
  selectCustomerServiceUserSocket(
    customer_service_user: CustomerService
  ): void {
    if (this.selectedCustomerServiceUser !== customer_service_user) {
      if (this.stompClient) {
        ...
        const conversationsCustomerServiceId = this.conversations?.map(
          (conversation) => {
            const conversationObj = {
              conversationid: conversation.id,
              customerserviceid:
                conversation.customerServiceModel.customerserviceid,
              customerid: conversation.customer.customerid,
            };
            return conversationObj;
          }
        );
        const isExistingConversation = conversationsCustomerServiceId?.find(
          (conv) =>
            conv.customerserviceid == customer_service.customerserviceid &&
            conv.customerid == customer?.customerid
        );
  ...
  createConversationSocket(conversationRequest: ConversationRequest): void {
    if (this.stompClient) {
      ...
      this.stompClient.send(
        `/app/create_private_conversation`,
        {},
  ...
  getAllCustomerServiceUsers() {
    ...
    this.getAllCustomerServiceUsersSocketSubscription =
    ...
    if (this.stompClient) {
      ...
      this.stompClient.send(
        `/app/get_all_customer_service_users`,
        {},
  ...
  findUserConversations(id: number | undefined): void {
    this.usersConversations = this.conversationsService
      .getAllConversations(id!)
      .subscribe((conversations: Conversation[]) => {
        this.conversations = conversations
          .map((conversation) => {
            let interlocutor!: string;
            this.loggedUser?.usertype == 'customer'
              ...
              : this.loggedUser?.usertype == 'customer_service'
              ? (interlocutor = `${conversation.customer.customer?.firstname} ${conversation.customer.customer?.lastname}`)
              : undefined;
  ...
  getConversationMessages(id: number | undefined): void {
    this.getConversationMessagesSubscription = this.chatService
      .get(id!)
  ...
  getCurrentConversation(id: number): void {
    this.selectConversationSubscription = this.conversationsService
      .getConversation(id)
      .subscribe((conversation: Conversation) => {
        ...
        this.loggedUser?.usertype == 'customer'
          ...
          : this.loggedUser?.usertype == 'customer_service'
          ? (interlocutor = `${this.currentConversation.customer.customer?.firstname} ${this.currentConversation.customer.customer?.firstname}`)
          : undefined;
